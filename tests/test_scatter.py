...
def test_scatter_max(data, token_size, num, dim, device):
    ...
    prediction = rua_scatter.scatter_max(tensor=tensor, index=index)
    target, _ = torch_scatter.scatter_max(src=tensor, index=index, dim=0)
...
def test_scatter_min(data, token_size, num, dim, device):
    ...
    prediction = rua_scatter.scatter_min(tensor=tensor, index=index)
    target, _ = torch_scatter.scatter_min(src=tensor, index=index, dim=0)
