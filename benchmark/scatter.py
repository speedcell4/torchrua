...
from torchrua.scatter import scatter_softmax as rua_scatter_softmax
...
def scatter_softmax(device: Type[draw_devices],
    ...
    with timer.rua_forward:
        prediction = rua_scatter_softmax(tensor=tensor, index=index)

    with timer.naive_forward:
        target = torch_scatter.scatter_softmax(src=tensor, index=index, dim=0)
